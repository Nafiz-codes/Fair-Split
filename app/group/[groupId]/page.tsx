...
import { createSettlement, getSettlements } from "@/app/actions/settlement";
import { computeBalances, simplifyDebts } from "@/lib/balances";

const stripeTestAccounts = ["acct_test_1 (Alice)", "acct_test_2 (Bob)", "acct_test_3 (Charlie)", "acct_test_4 (Dana)"];
...
export default async function GroupPage({ params, searchParams }: { params: { groupId: string }; searchParams?: { settled?: string } }) {
  ...
  async function recordSettlementFromForm(formData: FormData) {
    "use server";
    const settlement = await createSettlement(group.id, String(formData.get("fromMemberId") ?? ""), String(formData.get("toMemberId") ?? ""), Number(formData.get("amount")));
    revalidatePath(`/group/${group.id}`);
    redirect(`/group/${group.id}?settled=${settlement.id}`);
  }
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-10 text-slate-900 sm:px-6"><div className="mx-auto max-w-3xl space-y-6">
    ...
    <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8"><h2 className="text-xl font-bold">Settle Up</h2>{suggestedPayments.length === 0 ? <p className="mt-4 text-slate-600">Everyone is settled up.</p> : <div className="mt-5 space-y-3">{suggestedPayments.map((payment) => { const wasRecorded = settlements.some((settlement) => settlement.id === searchParams?.settled && settlement.fromMemberId === payment.fromId && settlement.toMemberId === payment.toId && settlement.amount === payment.amount); return <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-200 p-4" key={`${payment.fromId}-${payment.toId}-${payment.amount}`}><p className="font-medium">{payment.fromName} pays {payment.toName} ${payment.amount.toFixed(2)}</p>{wasRecorded ? <span className="rounded-full bg-emerald-100 px-3 py-1 text-sm font-medium text-emerald-700">Settlement recorded</span> : <form action={recordSettlementFromForm}><input name="fromMemberId" type="hidden" value={payment.fromId} /><input name="toMemberId" type="hidden" value={payment.toId} /><input name="amount" type="hidden" value={payment.amount} /><button className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700" type="submit">Record Settlement</button></form>}</div>})}</div>}</section>
